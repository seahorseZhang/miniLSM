...
TEST(SkipListCombined, FullWorkflow) {
    ...
    for(int i = 0; i < 20; i += 3)
        EXPECT_TRUE(sl.update(i, i * 111));

    for(int i = 0; i < 20; ++i) {
        auto v = sl.find(i);
        ASSERT_TRUE(v.has_value());
        if(i % 3 == 0)
            EXPECT_EQ(v.value(), i * 111);
        else
            EXPECT_EQ(v.value(), i * 11);
    }
...
TEST(SkipListBoundaryEmpty, BoundaryAndEmpty) {
    ...
    ASSERT_TRUE(vmin.has_value());
    EXPECT_EQ(vmin.value(), -1);
    ASSERT_TRUE(vmax.has_value());
    EXPECT_EQ(vmax.value(), 1);
}
