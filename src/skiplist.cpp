...
SkipList<K,V>::Node::Node(int level, const K& k, const V& v)
...
bool SkipList<K,V>::insert(const K& key, const V& value) {
    ...
    x = x->forward[0];
    if (x && x->key == key) return false; // already exists
    int lvl = randomLevel();
    if (lvl > level_) {
...
void SkipList<K,V>::clear() {
    ...
    for (int i = 0; i < maxLevel_; ++i) head_->forward[i].reset();
    level_ = 1;
    size_ = 0;
}

template<typename K, typename V>
size_t SkipList<K,V>::size() const { return size_; }
